...
def render_dashboard(campanha_id=None, show_instructions=True, set_page_config=True):
    """Render the dashboard into the current Streamlit app context.

    If `campanha_id` is None a campaign selector is shown (default behaviour).
    If `set_page_config` is False, page config is not set (useful when called from another page).
    """
    if set_page_config:
    ...
    if pagamentos:
        ...
        if 'bloco_id' in df_pag.columns and not df_blocos.empty:
            ...
            if not top.empty:
                ...
                st.plotly_chart(fig)
                st.dataframe(top.rename(columns={'nome':'Escuteiro','valor_pago':'Valor Recebido (€)','quantidade_rifas':'Rifas Confirmadas'}), hide_index=True)
            else:
    ...
    with st.expander('Lista de Blocos (detalhada)', expanded=False):
        if blocos:
            ...
            st.dataframe(df_blocos[['id','nome','numero_inicial','numero_final','rifas_total','escuteiro_id','seccao','estado','preco_bloco']])
        else:
    ...
    with st.expander('Pagamentos recentes', expanded=False):
        if pagamentos:
            ...
            st.dataframe(df_pag.sort_values('data_pagamento', ascending=False).head(50))
        else:
    ...
    # Return summary dict useful for tests/programmatic use
    return {
        'total_escuteiros': total_escuteiros,
        'total_blocos': total_blocos,
        'assigned_count': assigned_count,
        'confirmed_sold_blocks': confirmed_sold_blocks,
